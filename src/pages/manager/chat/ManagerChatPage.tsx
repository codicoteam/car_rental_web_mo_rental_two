...
export default function ManagerChatPage() {
  ...
  const selectedConvoRef = useRef<UIConversation | null>(null);
  ...
  useEffect(() => {
    ...
    s.on("connect", () => {
      if (selectedConvoRef.current?.id) s.emit("chat:join_conversation", { conversationId: selectedConvoRef.current.id });
    });
    s.on("chat:message_created", ({ message }: { message: IChatMessage }) => {
      ...
      setMessagesByConvo(prev => {
        ...
        const filtered = message.sender_id === myId
          ? arr.filter(m => !m.id.startsWith("temp_") || !(m.content === ui.content && Math.abs(m.createdAt.getTime() - ui.createdAt.getTime()) < 30_000))
          : arr;
      ...
      setConversations(prev => prev.map(c =>
        c.id === message.conversation_id
          ? { ...c, lastMessage: { content: message.content || "", createdAt: new Date(message.created_at) },
              unreadCount: selectedConvoRef.current?.id === message.conversation_id ? 0 : c.unreadCount + 1 }
          : c
      ));
    });
    ...
    s.on("typing:started", ({ conversationId, userId }: { conversationId: string; userId: string }) => {
      if (conversationId === selectedConvoRef.current?.id && userId !== myId) setTyping(true);
    });
    s.on("typing:stopped", ({ conversationId, userId }: { conversationId: string; userId: string }) => {
      if (conversationId === selectedConvoRef.current?.id && userId !== myId) setTyping(false);
    });
  ...
  const sendMessage = useCallback(async () => {
    ...
    setConversations(prev => prev.map(c =>
      c.id === selectedConvo.id ? { ...c, lastMessage: { content: `You: ${text}`, createdAt: new Date() } } : c
    ));
    setInputText("");
    try {
          ...
          <div className="flex border-b border-slate-100">
            {(["chats", "contacts"] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`flex-1 py-2.5 text-xs font-semibold transition-colors capitalize flex items-center justify-center gap-1.5 ${activeTab === tab ? "border-b-2" : "text-slate-500 hover:text-slate-700"}`}
                      ...
                      return (
                        <button key={u._id} onClick={() => startDirectWith(u)}
                          ...
                          <div className="flex items-center gap-1.5 flex-shrink-0">
                            {hasConvo && <div className="w-1.5 h-1.5 rounded-full" style={{ background: BRAND_BLUE }} />}
          ...
          {selectedConvo ? (
            ...
            <div className="flex-1 flex items-center justify-center bg-slate-50">
              <div className="text-center px-6">
                ...
                <h3 className="text-lg font-bold text-slate-800 mb-1">Manager Team Chat</h3>
                <p className="text-sm text-slate-500 mb-5 max-w-xs">
                  Communicate with admins, agents, drivers, receptionists, and executives.
                </p>
