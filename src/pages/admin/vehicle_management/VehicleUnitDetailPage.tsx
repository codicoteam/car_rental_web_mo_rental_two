...
import {
  ...
  DollarSign, Lock, TrendingUp, Plus, ExternalLink,
} from "lucide-react";
...
const VehicleUnitDetailPage: React.FC = () => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* LEFT: Photo gallery + unit details */}
            <div className="lg:col-span-2 space-y-5">
                  ...
                  <div>
                    ...
                    {photos.length > 1 && (
                      <div className="p-3 flex gap-2 overflow-x-auto">
                        {photos.map((p, i) => (
                          <button key={i} onClick={() => setPhotoIndex(i)}
                            className={`w-16 h-12 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-colors ${i === photoIndex ? "border-[#1EA2E4]" : "border-transparent"}`}>
              ...
              {/* Metadata */}
              {vehicle.metadata && (Object.keys(vehicle.metadata).filter(k => vehicle.metadata![k] !== undefined && vehicle.metadata![k] !== null)).length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-5 py-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {vehicle.metadata.notes && (
                      <div className="col-span-2">
                        ...
                        <p className="text-sm text-gray-700">{vehicle.metadata.notes}</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-5 py-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      navigate(`/admin-rate-plans?vehicleId=${vehicleId}&vehicleName=${encodeURIComponent(label || "")}`);
                    }}
                ...
                {plansLoading ? (
                  ...
                  <div className="py-4 text-center border-2 border-dashed border-gray-100 rounded-xl">
                    ...
                    <button
                      onClick={() => navigate(`/admin-rate-plans?vehicleId=${vehicleId}&vehicleName=${encodeURIComponent(vehicle.plate_number)}`)}
              ...
              {vehicle.accounting && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
                  <div className="flex items-center gap-2 mb-3">
                    <p className="text-xs font-semibold text-amber-800 uppercase tracking-wider">Financial Information</p>
                    <span className="flex items-center gap-1 px-1.5 py-0.5 bg-amber-100 border border-amber-200 text-amber-700 text-[10px] font-semibold rounded-full">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs text-amber-700 w-28 flex-shrink-0">Purchase Price</span>
                      <span className="text-sm font-bold text-gray-900">
                        {vehicle.accounting.purchase_price != null
                          ? `${vehicle.accounting.currency ?? ""} ${vehicle.accounting.purchase_price.toLocaleString("en-US", { minimumFractionDigits: 2 })}`
                          : "—"}
                      </span>
                    </div>
                    <div className="flex items-center gap-2.5">
                      <Calendar className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" />
                      <span className="text-xs text-amber-700 w-28 flex-shrink-0">Purchase Date</span>
                      <span className="text-sm font-medium text-gray-800">
                        {vehicle.accounting.purchased_at
                          ? new Date(vehicle.accounting.purchased_at).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
                          : "—"}
                      </span>
                    </div>
                  </div>
...
export default VehicleUnitDetailPage;
