...
export default function AdminAccountingPage() {
  ...
  const userRole = (() => {
    try { return JSON.parse(localStorage.getItem("car_rental_auth") || "{}")?.user?.roles?.[0] ?? null; }
    catch { return null; }
  })();
  const canEditBalanceSheet = userRole === "admin" || userRole === "manager";
  ...
  const buildQuery = useCallback(() => {
    ...
    if (selectedBranch) q.branch_id = selectedBranch;
    if (compareOn && compareFrom && compareTo) {
      q.compare_from = compareFrom;
      q.compare_to = compareTo;
    }
    return q;
  }, [fromDate, toDate, selectedBranch, compareOn, compareFrom, compareTo]);
  ...
  useEffect(() => {
    loadTab(activeTab);
  }, [activeTab, fromDate, toDate, selectedBranch, compareOn, compareFrom, compareTo]);
  ...
  function renderKPICard(
    ...
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        ...
        <div className={`text-2xl font-bold tabular-nums ${getMoneyClass(value)}`}>
          {displayVal}
        </div>
        {prior != null && (
            ...
            {variance && (
              <span
                className={`ml-2 font-semibold ${
                  variance.positive ? "text-emerald-600" : "text-red-600"
                }`}
              >
                {variance.positive ? "+" : ""}
                {variance.pct}%
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          ...
          {renderKPICard(
            ...
            kpis.net_profit >= 0 ? "bg-emerald-100" : "bg-red-100",
            pk?.net_profit
          )}
          {renderKPICard(
            "Gross Margin",
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex gap-2 no-print flex-wrap">
          ...
          <button
            onClick={() => exportTradingCSV(tradingData)}
            className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
          >
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm print-full max-w-3xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1">
              For the period: {period.from.slice(0, 10)} to {period.to.slice(0, 10)}
            </p>
            <p className="text-sm text-gray-500">Branch: {selectedBranchName}</p>
          </div>
          ...
          <StatLine
            label="Rental Income"
            value={revenue.rental_income}
            prior={hasPrior ? prior.revenue.rental_income : undefined}
          />
          <StatLine
            label="Driver Income"
            value={revenue.driver_income}
            prior={hasPrior ? prior.revenue.driver_income : undefined}
          />
          <StatLine
            label="Less: Promo Discounts"
            value={-revenue.promo_discounts}
            prior={hasPrior ? -prior.revenue.promo_discounts : undefined}
            indent
          />
          <StatLine
            label="Less: Refunds"
            value={-revenue.refunds}
            prior={hasPrior ? -prior.revenue.refunds : undefined}
            indent
          />
          <StatSubtotal
            label="Net Revenue"
            value={revenue.net_revenue}
            prior={hasPrior ? prior.revenue.net_revenue : undefined}
          />
          ...
          {cost_of_revenue.by_category.map((cat) => (
            <StatLine
              ...
              label={cat.label}
              value={cat.total}
              prior={hasPrior ? prior.cost_of_revenue.by_category.find((c) => c.category === cat.category)?.total : undefined}
              clickable
          ...
          <StatLine
            ...
            value={cost_of_revenue.service_orders}
            prior={hasPrior ? prior.cost_of_revenue.service_orders : undefined}
          />
          <StatLine
            label="Vehicle Incidents (net)"
            value={cost_of_revenue.incidents_net}
            prior={hasPrior ? prior.cost_of_revenue.incidents_net : undefined}
          />
          <StatSubtotal
            label="Total Cost of Revenue"
            value={cost_of_revenue.total}
            prior={hasPrior ? prior.cost_of_revenue.total : undefined}
          />
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div className="flex gap-6">
              <span className={`text-base font-bold tabular-nums w-28 text-right ${getMoneyClass(gross_profit)}`}>
                ${fmtMoney(gross_profit)}
              </span>
              {hasPrior && (
                <>
                  <span className={`text-sm font-semibold tabular-nums w-28 text-right ${getMoneyClass(prior.gross_profit)}`}>
                    ${fmtMoney(prior.gross_profit)}
                  </span>
            ...
            <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-2xl z-50 flex flex-col">
              ...
              <div className="overflow-y-auto flex-1 p-4">
                <div className="text-lg font-bold text-gray-900 mb-4">
                  Total: ${fmtMoney(drawerCategory.total)}
                </div>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {drawerExpenses.map((e: any) => (
                        <tr key={e._id}>
                          <td className="py-1.5 text-gray-600">{e.date?.slice(0, 10)}</td>
                          <td className="py-1.5 text-gray-700">{e.title}</td>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm print-full max-w-3xl">
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div className="flex gap-6">
              <span className={`text-base font-bold tabular-nums w-28 text-right ${getMoneyClass(net_profit)}`}>
                ${fmtMoney(net_profit)}
              </span>
              {hasPrior && (
                <>
                  <span className={`text-sm font-semibold tabular-nums w-28 text-right ${getMoneyClass(prior.net_profit)}`}>
                    ${fmtMoney(prior.net_profit)}
                  </span>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="relative flex-1 min-w-52">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1EA2E4]"
            />
          </div>
          <select
            value={ledgerLimit}
            onChange={(e) => { setLedgerLimit(Number(e.target.value)); setLedgerPage(1); }}
            className="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1EA2E4]"
          >
                  ...
                  filtered.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right tabular-nums font-medium text-emerald-600">
                        {row.amount_in > 0 ? `$${fmtMoney(row.amount_in)}` : "—"}
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums font-medium text-red-600">
                        {row.amount_out > 0 ? `$${fmtMoney(row.amount_out)}` : "—"}
                      </td>
  ...
  function renderRevenueAnalysis() {
    ...
    const { by_source, by_branch, trend, metrics } = revenueData;
    const pieData = [
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            ...
            <p className="text-xl font-bold text-gray-900 tabular-nums">${fmtMoney(metrics.avg_daily_revenue)}</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 mb-1">Peak Day</p>
            <p className="text-xl font-bold text-gray-900 tabular-nums">${fmtMoney(metrics.peak_day?.amount || 0)}</p>
            <p className="text-xs text-gray-400">{metrics.peak_day?.date || "—"}</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 mb-1">Total Transactions</p>
            <p className="text-xl font-bold text-gray-900 tabular-nums">{metrics.total_transactions}</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 mb-1">Avg Transaction Value</p>
            <p className="text-xl font-bold text-gray-900 tabular-nums">${fmtMoney(metrics.avg_transaction_value)}</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={trend} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="amount" stroke="#1EA2E4" strokeWidth={2} fill="url(#revGrad2)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          {by_branch.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm lg:col-span-2">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={by_branch} layout="vertical" margin={{ top: 4, right: 8, left: 80, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                  <XAxis type="number" tick={{ fontSize: 11 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                  <YAxis type="category" dataKey="branch_name" tick={{ fontSize: 11 }} />
                  <Tooltip formatter={(v: number) => [`$${fmtMoney(v)}`, "Revenue"]} />
                  <Bar dataKey="total_revenue" fill="#1EA2E4" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-lg font-bold text-gray-900 tabular-nums">${fmtMoney(by_source.rental_income)}</p>
              <p className="text-xs text-gray-400">{by_source.rental_count} rentals</p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Driver Income</p>
              <p className="text-lg font-bold text-gray-900 tabular-nums">${fmtMoney(by_source.driver_income)}</p>
              <p className="text-xs text-gray-400">{by_source.driver_count} bookings</p>
            </div>
          </div>
        </div>
      </div>
  ...
  function renderAuditTrail() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <button
            onClick={() => setAuditFlagLegendOpen((o) => !o)}
          ...
          {auditFlagLegendOpen && (
            <div className="px-5 pb-4 flex flex-wrap gap-2">
              {Object.entries(FLAG_INFO).map(([key, info]) => (
                <div key={key} className={`px-2.5 py-1 rounded-full text-xs font-medium ${info.color} ${info.bg}`}>
                  {key}: {info.label}
                </div>
              ))}
            </div>
        ...
        <div className="flex justify-end no-print">
          <button
            onClick={() => exportAuditCSV(auditData)}
            className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
          >
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-200">
            <h3 className="font-semibold text-gray-800">Expense Audit</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-gray-100">
                {expenses.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="text-center py-12 text-gray-400">No expenses in period</td>
                  </tr>
                ) : (
                  expenses.map((e: AuditExpense) => (
                    <>
                      <tr
                        key={e._id}
                        onClick={() => setExpandedExpenseRow(expandedExpenseRow === e._id ? null : e._id)}
                        className={`cursor-pointer hover:bg-gray-50 transition-colors ${
                          e.flags.includes("SELF_APPROVAL")
                            ? "bg-red-50"
                            : e.flags.length > 0
                            ? "bg-amber-50"
                            : ""
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {e.flags.map((flag) => {
                              ...
                              return (
                                <span key={flag} className={`px-1.5 py-0.5 rounded text-xs font-medium ${info?.color || "text-gray-700"} ${info?.bg || "bg-gray-100"}`}>
                                  {flag}
                                </span>
                              );
                            })}
                      ...
                      {expandedExpenseRow === e._id && (
                        <tr key={`${e._id}-expand`} className="bg-gray-50">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-600">
                              ...
                              {e.rejection_reason && (
                                <div className="col-span-4 text-red-600">
                                  <span className="font-medium">Rejection Reason:</span> {e.rejection_reason}
                                </div>
                              )}
                            </div>
                          </td>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-200">
            <h3 className="font-semibold text-gray-800">Payment Audit</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-gray-100">
                {payments.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-12 text-gray-400">No payments in period</td>
                  </tr>
                ) : (
                  payments.map((p: AuditPayment) => (
                    <tr
                      key={p._id}
                      className={
                        p.flags.includes("FAILED_ATTEMPT")
                          ? "bg-red-50"
                          : p.flags.includes("LARGE_PAYMENT")
                          ? "bg-amber-50"
                          : p.flags.includes("REFUND_ISSUED")
                          ? "bg-blue-50"
                          : ""
                      }
                    >
  ...
  function openDrawer(cat: CategoryLine) {
    ...
    setDrawerExpenses([]);
    setDrawerOpen(true);
    // Fetch drill-down expenses for this category in the current period
    ...
    fetch(`http://13.61.185.238:5050/api/v1/expenses?${params.toString()}`, {
  ...
  function exportTradingCSV(data: TradingAccountData) {
    const rows: string[][] = [
      ["MoRental Zimbabwe - Trading Account"],
      [`Period: ${data.period.from.slice(0, 10)} to ${data.period.to.slice(0, 10)}`],
      [],
      ["Section", "Item", "Amount"],
      ["Revenue", "Rental Income", String(data.revenue.rental_income)],
      ["Revenue", "Driver Income", String(data.revenue.driver_income)],
      ["Revenue", "Promo Discounts", String(-data.revenue.promo_discounts)],
      ["Revenue", "Refunds", String(-data.revenue.refunds)],
      ["Revenue", "Net Revenue", String(data.revenue.net_revenue)],
      ...data.cost_of_revenue.by_category.map((c) => ["Cost of Revenue", c.label, String(c.total)]),
      ["Cost of Revenue", "Service Orders", String(data.cost_of_revenue.service_orders)],
      ["Cost of Revenue", "Incidents (net)", String(data.cost_of_revenue.incidents_net)],
      ["Cost of Revenue", "Total", String(data.cost_of_revenue.total)],
      ["", "Gross Profit", String(data.gross_profit)],
      ["", "Gross Margin %", `${data.gross_margin_pct.toFixed(1)}%`],
    ];
  ...
  function exportIncomeCSV(data: IncomeStatementData) {
    const rows: string[][] = [
      ["MoRental Zimbabwe - Income Statement"],
      [`Period: ${data.period.from.slice(0, 10)} to ${data.period.to.slice(0, 10)}`],
      [],
      ["Section", "Item", "Amount"],
      ["Revenue", "Rental Income", String(data.revenue.rental_income)],
      ...
      ["Cost of Revenue", "Total", String(data.cost_of_revenue.total)],
      ["", "Gross Profit", String(data.gross_profit)],
      ...data.operating_expenses.by_category.map((c) => ["Operating Expenses", c.label, String(c.total)]),
      ["Operating Expenses", "Total", String(data.operating_expenses.total)],
      ["", "Net Profit", String(data.net_profit)],
      ["", "Net Margin %", `${data.net_margin_pct.toFixed(1)}%`],
    ];
  ...
  function exportLedgerCSV(rows: LedgerEntry[]) {
    const csvRows: string[][] = [
      ["Date", "Reference", "Description", "Type", "Category", "Money In", "Money Out", "Source", "Branch", "Recorded By"],
      ...rows.map((r) => [
        r.date, r.ref, r.description, r.type, r.category,
        String(r.amount_in), String(r.amount_out), r.source, r.branch_name, r.recorded_by,
      ]),
    ];
  ...
  function exportAuditCSV(data: any) {
    const rows: string[][] = [
      ["=== FLAGGED EXPENSES ==="],
      ["Date", "Ref", "Title", "Category", "Amount", "Status", "Submitted By", "Approved By", "Flags"],
      ...data.expenses
        .filter((e: any) => e.flags.length > 0)
        .map((e: any) => [
          e.date?.slice(0, 10) || "",
          e.reference || "",
          e.title,
          ...
          String(e.amount),
          e.status,
          e.submitted_by?.full_name || "",
          e.approved_by?.full_name || "",
          e.flags.join(", "),
        ]),
      [],
      ["=== FLAGGED PAYMENTS ==="],
      ["Date", "Status", "Provider", "Method", "Amount", "User", "Flags"],
      ...data.payments
        .filter((p: any) => p.flags.length > 0)
        .map((p: any) => [
          p.boughtAt?.slice(0, 10) || "",
          p.paymentStatus,
          p.provider,
          p.method,
          String(p.pricePaid),
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-4">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div className="w-9 h-9 rounded-xl bg-[#1EA2E4]/10 flex items-center justify-center">
                  <BookOpen className="w-5 h-5 text-[#1EA2E4]" />
                </div>
                <div>
                  <h1 className="text-lg font-bold text-gray-900">Accounting</h1>
                  <p className="text-xs text-gray-500">Financial Reports & Analysis</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex items-center gap-1.5 px-3 py-2 text-sm bg-[#1EA2E4] text-white rounded-lg hover:bg-[#1EA2E4]/90 transition-colors disabled:opacity-50"
                >
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded text-[#1EA2E4]"
                />
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-3 text-sm font-medium transition-colors border-b-2 -mb-px whitespace-nowrap ${
                    activeTab === tab.id
                      ? isTier2 ? "border-emerald-500 text-emerald-600" : "border-[#1EA2E4] text-[#1EA2E4]"
                      : "border-transparent text-gray-600 hover:text-gray-900"
                  }`}
                >
                  <Icon size={13} />
                  {tab.label}
                  {tab.id === "data_health" && <span className="text-[9px] bg-emerald-100 text-emerald-600 px-1 py-0.5 rounded font-semibold">T2</span>}
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === "fixed_assets" && (
            <FixedAssetsTab
              query={{ from: fromDate, to: toDate, branch_id: selectedBranch || undefined }}
              canEdit={true}
            />
          )}
...
function PLRow({
  ...
  const cls = `flex justify-between items-center py-0.5 ${bold ? "font-semibold" : ""} ${large ? "text-base" : "text-sm"}`;
  return (
    <div className={cls}>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className={`tabular-nums ${getMoneyClass(value)}`}>
          {value < 0
...
function StatSection({ title }: { title: string }) {
  return (
    <div className="text-xs font-semibold text-gray-400 uppercase tracking-widest mt-2 mb-1">
      {title}
    </div>
  );
}
...
function StatLine({
  ...
  const variance = hasPrior ? fmtVariance(value, prior!) : null;
  return (
    <div
      className={`flex items-center justify-between py-1 text-sm ${
        indent ? "pl-4" : ""
      } ${clickable ? "cursor-pointer hover:bg-gray-50 rounded px-1 -mx-1 transition-colors" : ""}`}
      onClick={onClick}
    >
      <span className={`text-gray-700 flex items-center gap-1 ${clickable ? "text-[#1EA2E4] hover:underline" : ""}`}>
        {label}
      </span>
      <div className="flex items-center gap-6">
        <span className={`tabular-nums w-28 text-right ${getMoneyClass(value)}`}>
          {value < 0 ? `($${fmtMoney(Math.abs(value))})` : `$${fmtMoney(value)}`}
        </span>
        {hasPrior && (
          <>
            <span className={`text-xs tabular-nums w-28 text-right ${getMoneyClass(prior!)}`}>
              {prior! < 0 ? `($${fmtMoney(Math.abs(prior!))})` : `$${fmtMoney(prior!)}`}
            </span>
...
function VarianceBadge({ current, prior }: { current: number; prior: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center text-xs font-semibold w-24 justify-end tabular-nums ${
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      <button
        onClick={onRetry}
        className="px-4 py-2 bg-[#1EA2E4] text-white rounded-lg text-sm hover:bg-[#1EA2E4]/90 transition-colors"
      >
        Retry
      </button>
    </div>
