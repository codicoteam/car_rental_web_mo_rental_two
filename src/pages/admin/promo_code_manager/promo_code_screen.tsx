...
import {
  ...
  MoreVertical, Building2, Tag, Calendar, Percent, Globe, RefreshCw,
  Car, Sparkles, Copy, Hash, BarChart3, Zap, Clock, TrendingUp,
  Save, ChevronRight, MapPin, Check, ArrowRight, Ticket,
} from "lucide-react";
...
const PromoCodeScreen: React.FC = () => {
  ...
  const toggleClass = (cls: VehicleClass) => {
    setFormData(p => {
      ...
      return {
        ...p,
        constraints: { ...p.constraints, allowed_classes: list.includes(cls) ? list.filter(c => c !== cls) : [...list, cls] },
      };
    });
  };
  ...
  const toggleBranch = (id: string) => {
    setFormData(p => {
      ...
      return {
        ...p,
        constraints: { ...p.constraints, branch_ids: list.includes(id) ? list.filter(b => b !== id) : [...list, id] },
      };
    });
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {filtered.map(code => {
                  ...
                  const branchNames = branches
                    .filter(b => code.constraints?.branch_ids?.includes(b._id))
                    .map(b => b.name);

                  const accentColor = isPercent ? "#059669" : "#2563eb";

                  return (
                    <div key={code._id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col overflow-hidden">
                      {/* Accent bar */}
                      ...
                      <div className="px-5 pt-4 pb-3">
                        <div className="flex items-center justify-between gap-2 mb-3">
                          <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border text-xs font-medium"
                          ...
                          <span className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-md ${code.active ? "text-emerald-700 bg-emerald-50" : "text-gray-500 bg-gray-100"}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${code.active ? "bg-emerald-500" : "bg-gray-400"}`} />
                        ...
                        <div className="flex items-baseline gap-1.5 mb-0.5">
                          ...
                          <span className="text-sm font-semibold text-gray-400">OFF</span>
                        </div>
                        <p className="text-xs text-gray-400">{isPercent ? "Percentage discount" : `Fixed ${code.currency || "USD"} discount`}</p>
                      </div>

                      {/* Code box */}
                      <div className="mx-5 mb-3 flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl px-3.5 py-2.5">
                        <span className="font-mono font-bold text-gray-800 text-sm tracking-widest">{code.code}</span>
                        <button onClick={() => copyToClipboard(code.code)} className="p-1 text-gray-400 hover:text-[#1EA2E4] rounded transition-colors">
                      ...
                      <div className="px-5 pb-4 flex-1 flex flex-col">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                          ...
                          {expiring && <span className="ml-auto text-amber-600 font-medium">Expiring soon</span>}
                        </div>
                        ...
                        <div className="mb-3">
                          <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                            <span>Usage</span>
                            <span className="font-semibold text-gray-700">{code.used_count || 0} / {code.usage_limit ?? "∞"}</span>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-1">
                            {pct !== null ? (
                              <div className="h-1 rounded-full bg-gray-400 transition-all" style={{ width: `${pct}%` }} />
                            ) : (
                              <div className="h-1 rounded-full bg-gray-200 w-0" />
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1 mb-3">
                          {branchNames.length === 0
                            ? <span className="flex items-center gap-1 text-xs text-gray-400"><Globe className="w-3 h-3" /> All branches</span>
                            : branchNames.slice(0, 3).map(n => (
                              <span key={n} className="flex items-center gap-1 px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">
                                <Building2 className="w-3 h-3" />{n}
                              </span>
                            ))
                          }
                          {branchNames.length > 3 && <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-xs">+{branchNames.length - 3}</span>}
                          {code.constraints?.min_days && <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-xs">Min {code.constraints.min_days}d</span>}
                        </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="overflow-y-auto p-6 space-y-5" style={{ maxHeight: "calc(90vh - 200px)" }}>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                {usagePct(selectedPromo) !== null && (
                  <div className="mt-3">
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div className="h-2 rounded-full bg-[#1EA2E4]" style={{ width: `${usagePct(selectedPromo)}%` }} />
                    </div>
                    <p className="text-xs text-gray-500 mt-1 text-right">{Math.round(usagePct(selectedPromo) || 0)}% used</p>
                  </div>
                )}
              </div>
              ...
              {/* Constraints */}
              {((selectedPromo.constraints?.allowed_classes?.length ?? 0) > 0 || selectedPromo.constraints?.min_days) && (
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Restrictions</p>
                  {(selectedPromo.constraints?.allowed_classes?.length ?? 0) > 0 && (
                    <div className="mb-3">
                      <p className="text-xs text-gray-500 mb-2">Vehicle Classes</p>
                      <div className="flex flex-wrap gap-1.5">
                        {selectedPromo.constraints!.allowed_classes!.map(c => (
                          <span key={c} className="px-2.5 py-1 bg-violet-100 text-violet-700 rounded-full text-xs font-medium capitalize">{c}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {selectedPromo.constraints?.min_days && (
                    <p className="text-sm text-gray-700">Minimum <span className="font-bold">{selectedPromo.constraints.min_days}</span> rental days required</p>
                  )}
                </div>
              )}
              ...
              <div className="flex-1 overflow-y-auto px-8 py-6 space-y-7">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium text-sm flex items-center gap-2 transition-colors"
                        >
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {([
                          { val: "percent", label: "Percentage", desc: "e.g. 20% off", icon: Percent, gradient: "from-violet-500 to-indigo-600" },
                          { val: "fixed", label: "Fixed Amount", desc: "e.g. $15 off", icon: Tag, gradient: "from-emerald-500 to-teal-600" },
                        ] as const).map(t => (
                          <button
                            key={t.val}
                            type="button"
                            ...
                            style={formData.type === t.val ? { background: `linear-gradient(to bottom right, ${t.val === "percent" ? "#7c3aed, #4338ca" : "#10b981, #0d9488"})` } : {}}
                          >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">
                          {formData.type === "percent" ? "Percentage" : "Amount"} <span className="text-red-500">*</span>
                        </label>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Max Redemptions (Optional)</label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => setFormData(p => ({ ...p, usage_limit: e.target.value ? parseInt(e.target.value) : null }))}
                          className="w-full pl-9 pr-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1EA2E4]"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => setFormData(p => ({ ...p, constraints: { ...p.constraints, min_days: e.target.value ? parseInt(e.target.value) : undefined } }))}
                          className="w-full pl-9 pr-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1EA2E4]"
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Select specific branches or leave all unchecked for all branches</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <button
                      type="button"
                      onClick={() => setFormData(p => ({ ...p, constraints: { ...p.constraints, branch_ids: branches.map(b => b._id) } }))}
                      className="px-3 py-1.5 text-xs font-medium bg-blue-50 text-[#1EA2E4] rounded-lg hover:bg-blue-100 transition-colors"
                    >
                      Select All
                    </button>
                    <button
                      type="button"
                      onClick={() => setFormData(p => ({ ...p, constraints: { ...p.constraints, branch_ids: [] } }))}
                    ...
                    {(formData.constraints?.branch_ids?.length ?? 0) > 0 && (
                      <span className="text-xs text-emerald-600 font-medium">
                        {formData.constraints?.branch_ids?.length} branch{(formData.constraints?.branch_ids?.length ?? 0) !== 1 ? "es" : ""} selected
                      </span>
                    )}
                    {(formData.constraints?.branch_ids?.length ?? 0) === 0 && (
                      <span className="text-xs text-gray-400">All branches (no restriction)</span>
                    )}
                  </div>

                  {branches.length === 0 ? (
                    <div className="text-sm text-gray-400 bg-gray-50 rounded-xl p-4 text-center">Loading branches...</div>
                  ) : (
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Restrict to specific classes or leave empty for all</p>
                    </div>
                  </div>
                    ...
                    {vehicleClasses.map(cls => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl border-2 text-sm font-medium transition-all ${
                            sel ? "border-violet-500 bg-violet-500 text-white shadow-sm" : "border-gray-200 bg-white text-gray-600 hover:border-violet-300"
                          }`}
                        >
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    {(formData.constraints?.allowed_classes?.length ?? 0) === 0
                      ? "All vehicle classes allowed"
                      : `${formData.constraints?.allowed_classes?.length} class${(formData.constraints?.allowed_classes?.length ?? 0) !== 1 ? "es" : ""} selected`}
                  </p>
                </div>
                ...
                <div>
                  ...
                  >
                    <div className={`w-12 h-6 rounded-full relative transition-colors ${formData.active ? "bg-emerald-500" : "bg-gray-300"}`}>
                      <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${formData.active ? "left-6.5" : "left-0.5"}`} style={{ left: formData.active ? "26px" : "2px" }} />
                    </div>
                    <div className="text-left">
                      <p className={`font-semibold text-sm ${formData.active ? "text-emerald-700" : "text-gray-600"}`}>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
            ...
            <p className="text-sm text-gray-500 text-center mb-6">This action cannot be undone. The code will be permanently removed.</p>
