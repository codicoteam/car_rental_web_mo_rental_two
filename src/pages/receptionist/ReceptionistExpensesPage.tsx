...
> = {
  ...
  pending_approval: { label: "Pending Approval", color: "text-amber-700", bg: "bg-amber-100", icon: <Clock className="w-3 h-3" /> },
  approved: { label: "Approved", color: "text-green-700", bg: "bg-green-100", icon: <CheckCircle className="w-3 h-3" /> },
...
function getCatColor(cat: string): string {
  return CATEGORY_COLORS[cat] ?? "text-gray-600 bg-gray-100";
}

function getBranchName(branch_id: IExpense["branch_id"]): string {
  if (!branch_id) return "-";
  if (typeof branch_id === "object") return (branch_id as { name?: string }).name ?? "-";
  return "-";
}
...
async function uploadReceipt(file: File): Promise<string> {
  ...
  const { error } = await supabase.storage.from("topics").upload(fileName, file, { cacheControl: "3600", upsert: false });
...
const DEFAULT_FORM: ExpenseForm = {
  title: "", category: "", amount: "", currency: "USD",
  date: new Date().toISOString().split("T")[0],
  description: "", branch_id: "", vehicle_id: "", tags: "", notes: "",
  receipt_images: [], submitImmediately: false,
};

// ─────────────────────────────────────────────
// Component — Receptionist cannot approve
// ─────────────────────────────────────────────
const ReceptionistExpensesPage: React.FC = () => {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try { const data = await fetchExpenseSummary(token, userBranchId || undefined); setSummary(data); }
    catch { /* silent */ }
  ...
  const openCreateModal = () => { setEditingExpense(null); setForm({ ...DEFAULT_FORM, branch_id: userBranchId }); setFormErrors({}); setShowModal(true); };

  const openEditModal = (expense: IExpense) => {
    setEditingExpense(expense);
    const branchId = expense.branch_id && typeof expense.branch_id === "object" ? (expense.branch_id as { _id: string })._id : (expense.branch_id as string) || "";
    const vehicleId = expense.vehicle_id && typeof expense.vehicle_id === "object" ? (expense.vehicle_id as { _id: string })._id : (expense.vehicle_id as string) || "";
    setForm({
      title: expense.title, category: expense.category,
      amount: String(parseAmount(expense.amount)), currency: expense.currency,
      date: expense.date ? expense.date.split("T")[0] : "",
      description: expense.description || "", branch_id: branchId, vehicle_id: vehicleId,
      tags: (expense.tags || []).join(", "), notes: expense.notes || "",
      receipt_images: expense.receipt_images || [], submitImmediately: false,
    });
    setFormErrors({});
    setShowModal(true);
  };
  ...
  const handleSaveExpense = async () => {
    ...
    try {
      ...
      const payload: Partial<IExpense> = {
        title: form.title.trim(), category: form.category as ExpenseCategory,
        amount: Number(form.amount) as unknown as IExpense["amount"],
        currency: form.currency, date: new Date(form.date).toISOString(),
        description: form.description, branch_id: form.branch_id,
        vehicle_id: form.vehicle_id || undefined, receipt_images: form.receipt_images,
  ...
  const handleSubmitExpense = async (expense: IExpense) => {
    try {
      ...
      loadExpenses();
      loadSummary();
    } catch { showSnack("Failed to submit expense", "error"); }
  };
  ...
  const handleDelete = async (expense: IExpense) => {
    ...
    try {
      ...
      loadExpenses();
      loadSummary();
    } catch { showSnack("Failed to delete expense", "error"); }
  };
  ...
  // Receptionist: can only edit own drafts
  ...
  const canDelete = (expense: IExpense) =>
    getSubmittedById(expense.submitted_by) === userId && expense.status === "draft";
    ...
    <div className="flex min-h-screen bg-[#F0F6FF]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Create and submit expense entries</p>
            </div>
          </div>
        ...
        <main className="flex-1 p-6 space-y-6">
          {/* Stats — simpler for receptionist (3 cards, no approval-based stats) */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              ...
              <p className="text-xs text-amber-600 mt-1 font-medium">Awaiting review</p>
            </div>
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-3">
          ...
          {/* Info banner — receptionist cannot approve */}
          <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 border border-blue-100 rounded-xl">
            <AlertCircle className="w-4 h-4 text-blue-500 flex-shrink-0" />
            <p className="text-xs text-blue-700 font-medium">
              You can create and submit expenses. Approvals are handled by managers and admins.
            </p>
          </div>
                        ...
                        <tr key={expense._id} className={`border-b border-gray-50 hover:bg-blue-50/30 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/40"}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              {canSubmit(expense) && (
                                <button onClick={() => handleSubmitExpense(expense)} className="p-1.5 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-blue-600" title="Submit for approval">
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative bg-white h-full w-full max-w-2xl flex flex-col shadow-2xl">
            <div className="shrink-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10">
              ...
              <button onClick={() => !formSaving && setShowModal(false)} className="p-2 rounded-xl hover:bg-gray-100"><X className="w-5 h-5 text-gray-500" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div>
                ...
                <input type="text" value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="e.g., Office cleaning supplies"
          ...
          <div className="relative w-full max-w-xl bg-white h-full overflow-y-auto shadow-2xl">
            ...
            <div className="p-6 space-y-6">
              ...
              {detailExpense.rejection_reason && (
                <div><p className="text-xs text-red-400 uppercase font-semibold tracking-wide mb-1">Rejection Reason</p>
              ...
              <div className="flex flex-wrap gap-3 pt-2 border-t border-gray-100">
                ...
                {canEdit(detailExpense) && (
                  <button onClick={() => { setShowDetail(false); openEditModal(detailExpense); }} className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 text-sm font-medium">
...
export default ReceptionistExpensesPage;
