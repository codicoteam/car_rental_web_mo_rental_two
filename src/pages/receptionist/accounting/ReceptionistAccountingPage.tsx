...
function computePresetDates(preset: Preset): { from: Date; to: Date } {
  ...
  switch (preset) {
    ...
    case "last_month": { const first = new Date(now.getFullYear(), now.getMonth() - 1, 1); const last = new Date(now.getFullYear(), now.getMonth(), 0); return { from: first, to: last }; }
...
const TABS = [
  ...
  { id: "cash_flow", label: "Cash Flow", icon: Waves },
...
export default function ReceptionistAccountingPage() {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<TabId>("overview");

  const [preset, setPreset] = useState<Preset>("this_month");
  const [fromDate, setFromDate] = useState<string>(fmtDate(new Date(new Date().getFullYear(), new Date().getMonth(), 1)));
  const [toDate, setToDate] = useState<string>(fmtDate(new Date()));
  const [compareOn, setCompareOn] = useState(false);
  const [compareFrom, setCompareFrom] = useState<string>(() => { const d = new Date(); return fmtDate(new Date(d.getFullYear(), d.getMonth() - 1, 1)); });
  const [compareTo, setCompareTo] = useState<string>(() => { const d = new Date(); return fmtDate(new Date(d.getFullYear(), d.getMonth(), 0)); });
  ...
  const buildQuery = useCallback(() => {
    ...
    if (compareOn && compareFrom && compareTo) { q.compare_from = compareFrom; q.compare_to = compareTo; }
    return q;
  ...
  const loadTab = useCallback(async (tab: TabId) => {
    ...
    try {
      const q = buildQuery();
      if (tab === "overview") { const d = await fetchAccountingOverview(q as any); setOverviewData(d); }
      else if (tab === "trading") { const d = await fetchTradingAccount(q as any); setTradingData(d); }
      else if (tab === "income") { const d = await fetchIncomeStatement(q as any); setIncomeData(d); }
      else if (tab === "ledger") { const d = await fetchLedger({ ...q, page: ledgerPage, limit: ledgerLimit } as any); setLedgerData(d); }
      else if (tab === "revenue") { const d = await fetchRevenueAnalysis(q as any); setRevenueData(d); }
      else if (tab === "audit") { const d = await fetchAuditTrail(q as any); setAuditData(d); }
    } catch (e: any) {
      setError(e?.response?.data?.message || e?.message || "Failed to load data");
    } finally { setLoading(false); }
  }, [buildQuery, ledgerPage, ledgerLimit]);

  useEffect(() => { loadTab(activeTab); }, [activeTab, fromDate, toDate, compareOn, compareFrom, compareTo]);
  useEffect(() => { if (activeTab === "ledger") loadTab("ledger"); }, [ledgerPage, ledgerLimit]);
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        ...
        {prior != null && (
          <div className="mt-1.5 text-xs text-gray-500">
            ...
            {variance && <span className={`ml-2 font-semibold ${variance.positive ? "text-emerald-600" : "text-red-600"}`}>{variance.positive ? "+" : ""}{variance.pct}%</span>}
          </div>
        )}
      </div>
    );
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={revenue_trend} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <defs><linearGradient id="rRevGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#1EA2E4" stopOpacity={0.3} /><stop offset="95%" stopColor="#1EA2E4" stopOpacity={0} /></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(d) => d.slice(5)} />
                <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                <Tooltip formatter={(v: number) => [`$${fmtMoney(v)}`, "Revenue"]} />
                <Area type="monotone" dataKey="amount" stroke="#1EA2E4" strokeWidth={2} fill="url(#rRevGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={expense_trend} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <defs><linearGradient id="rExpGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ef4444" stopOpacity={0.3} /><stop offset="95%" stopColor="#ef4444" stopOpacity={0} /></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(d) => d.slice(5)} />
                <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                <Tooltip formatter={(v: number) => [`$${fmtMoney(v)}`, "Expenses"]} />
                <Area type="monotone" dataKey="amount" stroke="#ef4444" strokeWidth={2} fill="url(#rExpGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex gap-2 no-print flex-wrap">
          ...
          <button onClick={() => { const rows: string[][] = [["MoRental Zimbabwe - Trading Account"], [`Period: ${period.from.slice(0, 10)} to ${period.to.slice(0, 10)}`], [], ["Section", "Item", "Amount"], ["Revenue", "Net Revenue", String(revenue.net_revenue)], ...cost_of_revenue.by_category.map((c) => ["Cost of Revenue", c.label, String(c.total)]), ["Cost of Revenue", "Total", String(cost_of_revenue.total)], ["", "Gross Profit", String(gross_profit)]]; downloadCSV(rows, `trading_account.csv`); }} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Download className="w-4 h-4" /> Export CSV</button>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm print-full max-w-3xl">
          ...
          {hasPrior && <div className="flex justify-end gap-6 mb-2 text-xs font-semibold text-gray-500 uppercase tracking-wide pr-2"><span className="w-28 text-right">Current</span><span className="w-28 text-right">Prior</span><span className="w-24 text-right">Variance</span></div>}
          ...
          {cost_of_revenue.by_category.map((cat) => <StatLine key={cat.category} label={cat.label} value={cat.total} prior={hasPrior ? prior.cost_of_revenue.by_category.find((c) => c.category === cat.category)?.total : undefined} clickable onClick={() => openDrawer(cat)} />)}
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div className="flex gap-6">
              ...
              {hasPrior && <><span className={`text-sm font-semibold tabular-nums w-28 text-right ${getMoneyClass(prior.gross_profit)}`}>${fmtMoney(prior.gross_profit)}</span><VarianceBadge current={gross_profit} prior={prior.gross_profit} /></>}
            </div>
          </div>
          <div className="text-sm text-gray-500 text-right pr-2">Margin: {fmtPct(gross_margin_pct)}{hasPrior && <span className="ml-4">Prior: {fmtPct(prior.gross_margin_pct)}</span>}</div>
        </div>
          ...
          <>
            ...
            <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-2xl z-50 flex flex-col">
              <div className="flex items-center justify-between p-4 border-b border-gray-200">
                <div><h3 className="font-semibold text-gray-900">{drawerCategory.label}</h3><p className="text-xs text-gray-500">{drawerCategory.count} expense(s)</p></div>
              ...
              <div className="overflow-y-auto flex-1 p-4">
                ...
                {drawerExpenses.length === 0 ? <p className="text-sm text-gray-400">No details available.</p> : (
                  <table className="w-full text-xs"><thead><tr className="text-left text-gray-500 border-b border-gray-200"><th className="pb-2">Date</th><th className="pb-2">Title</th><th className="pb-2 text-right">Amount</th></tr></thead>
                  <tbody className="divide-y divide-gray-100">{drawerExpenses.map((e: any) => <tr key={e._id}><td className="py-1.5 text-gray-600">{e.date?.slice(0, 10)}</td><td className="py-1.5 text-gray-700">{e.title}</td><td className="py-1.5 text-right font-mono tabular-nums">${fmtMoney(e.amount)}</td></tr>)}</tbody></table>
                )}
              </div>
            </div>
          </>
  ...
  function renderIncomeStatement() {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex gap-2 no-print flex-wrap">
          <button onClick={() => window.print()} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Printer className="w-4 h-4" /> Print</button>
          <button onClick={() => { const rows: string[][] = [["MoRental Zimbabwe - Income Statement"], [`Period: ${period.from.slice(0, 10)} to ${period.to.slice(0, 10)}`], [], ["Net Revenue", String(revenue.net_revenue)], ["Gross Profit", String(gross_profit)], ["Net Profit", String(net_profit)]]; downloadCSV(rows, `income_statement.csv`); }} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Download className="w-4 h-4" /> Export CSV</button>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm print-full max-w-3xl">
          <div className="text-center mb-8">
            <h2 className="text-xl font-bold text-gray-900">MoRental Zimbabwe</h2>
          ...
          <div className="my-4" />
          <StatSection title="COST OF REVENUE" />
          {cost_of_revenue.by_category.map((cat) => <StatLine key={cat.category} label={cat.label} value={cat.total} prior={hasPrior ? prior.cost_of_revenue.by_category.find((c) => c.category === cat.category)?.total : undefined} />)}
          <StatLine label="Service Orders" value={cost_of_revenue.service_orders} prior={hasPrior ? prior.cost_of_revenue.service_orders : undefined} />
          <StatLine label="Vehicle Incidents (net)" value={cost_of_revenue.incidents_net} prior={hasPrior ? prior.cost_of_revenue.incidents_net : undefined} />
          <StatSubtotal label="Total Cost of Revenue" value={cost_of_revenue.total} prior={hasPrior ? prior.cost_of_revenue.total : undefined} />
          ...
          {operating_expenses.by_category.map((cat) => <StatLine key={cat.category} label={cat.label} value={cat.total} prior={hasPrior ? prior.operating_expenses.by_category.find((c) => c.category === cat.category)?.total : undefined} />)}
          ...
          <div className="flex items-center justify-between py-2">
            ...
            <div className="flex gap-6">
              ...
              {hasPrior && <><span className={`text-sm font-semibold tabular-nums w-28 text-right ${getMoneyClass(prior.net_profit)}`}>${fmtMoney(prior.net_profit)}</span><VarianceBadge current={net_profit} prior={prior.net_profit} /></>}
            </div>
          </div>
          <div className="text-sm text-gray-500 text-right pr-2">Net Margin: {fmtPct(net_margin_pct)}{hasPrior && <span className="ml-4">Prior: {fmtPct(prior.net_margin_pct)}</span>}</div>
        </div>
      </div>
    );
  }
  ...
  function renderLedger() {
    ...
    if (ledgerSearch.trim()) { const s = ledgerSearch.toLowerCase(); filtered = filtered.filter((r) => r.description.toLowerCase().includes(s) || r.ref.toLowerCase().includes(s) || r.category.toLowerCase().includes(s)); }
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            {["all", "revenue", "expense", "service_cost"].map((t) => <button key={t} onClick={() => setLedgerType(t)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${ledgerType === t ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}>{t === "all" ? "All" : t === "service_cost" ? "Service Cost" : t.charAt(0).toUpperCase() + t.slice(1)}</button>)}
          </div>
          <div className="relative flex-1 min-w-52"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input value={ledgerSearch} onChange={(e) => setLedgerSearch(e.target.value)} placeholder="Search..." className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1EA2E4]" /></div>
          <select value={ledgerLimit} onChange={(e) => { setLedgerLimit(Number(e.target.value)); setLedgerPage(1); }} className="border border-gray-200 rounded-lg text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1EA2E4]"><option value={25}>25/page</option><option value={50}>50/page</option><option value={100}>100/page</option></select>
          <button onClick={() => { const csvRows: string[][] = [["Date", "Reference", "Description", "Type", "Category", "Money In", "Money Out", "Source"], ...rows.map((r) => [r.date, r.ref, r.description, r.type, r.category, String(r.amount_in), String(r.amount_out), r.source])]; downloadCSV(csvRows, `ledger_export.csv`); }} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Download className="w-4 h-4" /> Export</button>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wide"><tr><th className="px-4 py-3 text-left">Date</th><th className="px-4 py-3 text-left">Reference</th><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-left">Type</th><th className="px-4 py-3 text-left">Category</th><th className="px-4 py-3 text-right">Money In</th><th className="px-4 py-3 text-right">Money Out</th><th className="px-4 py-3 text-left">Source</th></tr></thead>
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 ? <tr><td colSpan={8} className="text-center py-12 text-gray-400">No transactions found</td></tr> : (
                  filtered.map((row, idx) => <tr key={idx} className={`hover:bg-gray-50 transition-colors ${row.type === "revenue" ? "border-l-2 border-emerald-400" : row.type === "expense" ? "border-l-2 border-red-400" : "border-l-2 border-orange-400"}`}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2"><button disabled={ledgerPage <= 1} onClick={() => setLedgerPage((p) => p - 1)} className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50">Previous</button><button disabled={ledgerPage >= totalPages} onClick={() => setLedgerPage((p) => p + 1)} className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50">Next</button></div>
        </div>
      </div>
    );
  }
  ...
  function renderRevenueAnalysis() {
    ...
    const pieData = [{ name: "Rental Income", value: by_source.rental_income }, { name: "Driver Income", value: by_source.driver_income }].filter((d) => d.value > 0);
    return (
      <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={trend} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <defs><linearGradient id="rRevGrad2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#1EA2E4" stopOpacity={0.3} /><stop offset="95%" stopColor="#1EA2E4" stopOpacity={0} /></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(d) => d.slice(5)} />
                <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
                <Tooltip formatter={(v: number) => [`$${fmtMoney(v)}`, "Revenue"]} />
                <Area type="monotone" dataKey="amount" stroke="#1EA2E4" strokeWidth={2} fill="url(#rRevGrad2)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
  ...
  function renderAuditTrail() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <button onClick={() => setAuditFlagLegendOpen((o) => !o)} className="w-full flex items-center justify-between px-5 py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50"><span>Flag Legend</span>{auditFlagLegendOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button>
          {auditFlagLegendOpen && <div className="px-5 pb-4 flex flex-wrap gap-2">{Object.entries(FLAG_INFO).map(([key, info]) => <div key={key} className={`px-2.5 py-1 rounded-full text-xs font-medium ${info.color} ${info.bg}`}>{key}: {info.label}</div>)}</div>}
        </div>
        <div className="flex justify-end no-print">
          <button onClick={() => { const rows: string[][] = [["=== FLAGGED EXPENSES ==="], ["Date", "Ref", "Title", "Amount", "Status", "Flags"], ...auditData.expenses.filter((e: any) => e.flags.length > 0).map((e: any) => [e.date?.slice(0, 10) || "", e.reference || "", e.title, String(e.amount), e.status, e.flags.join(", ")])]; downloadCSV(rows, `audit_report.csv`); }} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Download className="w-4 h-4" /> Export Audit Report</button>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-200"><h3 className="font-semibold text-gray-800">Expense Audit</h3></div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wide"><tr><th className="px-4 py-3 text-left">Date</th><th className="px-4 py-3 text-left">Ref</th><th className="px-4 py-3 text-left">Title</th><th className="px-4 py-3 text-left">Category</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3 text-left">Status</th><th className="px-4 py-3 text-left">Submitted By</th><th className="px-4 py-3 text-left">Approved By</th><th className="px-4 py-3 text-left">Flags</th></tr></thead>
              <tbody className="divide-y divide-gray-100">
                        ...
                        <tr key={`${e._id}-exp`} className="bg-gray-50">
                          <td colSpan={9} className="px-6 py-3">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-600">
                              ...
                              {e.rejection_reason && <div className="col-span-4 text-red-600"><span className="font-medium">Rejection:</span> {e.rejection_reason}</div>}
                            </div>
                          </td>
                        </tr>
                      )}
  ...
  function openDrawer(cat: CategoryLine) {
    setDrawerCategory(cat); setDrawerExpenses([]); setDrawerOpen(true);
    ...
    fetch(`http://13.61.185.238:5050/api/v1/expenses?${params.toString()}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then((r) => r.json()).then((data) => setDrawerExpenses(data.data || [])).catch(() => {});
  }
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-gray-100"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg></button>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleRefresh} disabled={loading} className="flex items-center gap-1.5 px-3 py-2 text-sm bg-[#1EA2E4] text-white rounded-lg hover:bg-[#1EA2E4]/90 transition-colors disabled:opacity-50"><RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} /> Refresh</button>
              </div>
            </div>
            <div className="flex flex-wrap gap-3 items-end">
              <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
                {(["this_month", "last_month", "this_quarter", "this_year", "last_year", "custom"] as Preset[]).map((p) => <button key={p} onClick={() => setPreset(p)} className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${preset === p ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}>{p === "this_month" ? "This Month" : p === "last_month" ? "Last Month" : p === "this_quarter" ? "This Quarter" : p === "this_year" ? "This Year" : p === "last_year" ? "Last Year" : "Custom"}</button>)}
              </div>
              {preset === "custom" && (
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-1.5 px-3 py-3 text-sm font-medium transition-colors border-b-2 -mb-px whitespace-nowrap ${activeTab === tab.id ? "border-[#1EA2E4] text-[#1EA2E4]" : "border-transparent text-gray-600 hover:text-gray-900"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === "fixed_assets" && <FixedAssetsTab query={{}} canEdit={false} />}
          {activeTab === "balance_sheet" && <BalanceSheetTab query={{}} canEdit={false} />}
  ...
  return (
    <div className={`flex items-center justify-between py-1 text-sm ${indent ? "pl-4" : ""} ${clickable ? "cursor-pointer hover:bg-gray-50 rounded px-1 -mx-1 transition-colors" : ""}`} onClick={onClick}>
      ...
      <div className="flex items-center gap-6">
        ...
        {hasPrior && <><span className={`text-xs tabular-nums w-28 text-right ${getMoneyClass(prior!)}`}>{prior! < 0 ? `($${fmtMoney(Math.abs(prior!))})` : `$${fmtMoney(prior!)}`}</span><VarianceBadge current={value} prior={prior!} /></>}
      </div>
    </div>
  );
}
